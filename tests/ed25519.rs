...
fn golden() {
    let testdata_path = {
        let root = std::env::var("CARGO_MANIFEST_DIR").expect("failed read 'CARGO_MANIFEST_DIR'");
...
fn malleability() {
    ...
    let pub_key = PublicKey::try_from(&raw_pub_key[..]).expect("failed to unmarshal public key");
